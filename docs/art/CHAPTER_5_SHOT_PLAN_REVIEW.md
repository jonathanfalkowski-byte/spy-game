...
H1 `c05.s06.shot01-preview`: **PASS with LOW/POLISH**, candidate `c5-h1-arrival-composite-v3-staging`, human production approval pending. H2 `c05.s06.shot13-return`: **PASS with LOW/POLISH**, candidate `c5-h2-coffee-return-composite-v1-staging`, human production approval pending. H3/H4 remain approved exact shots. No promotion: approved coverage remains **4/95 (4.21%)**, three runtime-bound Chapter 5 shots. Pilot spend is **22 credits** (one H2 layer and one failed Daytime Apartment character-only correction added). See [full review](HARBOUR_READER_V2_REVIEW.md), the [definition of done](CHAPTER_5_VISUAL_DEFINITION_OF_DONE.md), and staging catalog records; these current statuses supersede earlier host-missing/revise and spend statements without replacing historical evidence.

## Harbour closure — 2026-09-18

`c05.s06.shot01-preview` now resolves to production asset `c5-h1-arrival-composite-v3-production` for the reviewed evening/professional arrival/programme-table state. `c05.s06.shot13-return` now resolves to production asset `c5-h2-coffee-return-composite-v1-production` for the reviewed post-Julian return with one Evelynn-held coffee. Both remain offline catalog entries pending a separately authorized runtime-binding change.

H1/H2/H3/H4 complete the only Chapter 5 Harbour visual families. Holds stay within those exact states; no extra Harbour movement, daylight/courtyard branch, or alternative angle is authorized.
