...
it('freezes every revision-13 dependency byte against the source commit', () => {
  ...
  const originals = execFileSync('git', ['cat-file', '--batch'], {
    input:
  ...
  for (const [file, hash] of Object.entries(manifest.files)) {
    ...
    const end = originals.indexOf(10, offset);
    const header = originals.subarray(offset, end).toString('utf8').split(' ');
    expect(header[1]).toBe('blob');
    const size = Number(header[2]);
    expect(bytes.equals(originals.subarray(end + 1, end + 1 + size)), file).toBe(true);
    offset = end + size + 2;
  }
}, 30000);
...
it('keeps consent and sourced knowledge separate and replays schema 5', () => {
  ...
  expect(s.clinic).toEqual(before.clinic);
  expect(s.relationships).toEqual(before.relationships);
  expect(s.npcs.sloane).toEqual(before.npcs.sloane);
  expect(
    s.npcs.voss.known.slice(before.npcs.voss.known.length).some((x) => /Maya|sender/.test(x.key)),
  ).toBe(false);
...
it('registers approved Julian canon without relationship assumptions', () => {
  ...
  expect(c.displayName).toBe('Julian Mercer');
  expect(c.canon.age?.years).toBe(49);
  expect(c.introduction.role).toBe('Helix Group COO');
  expect(c.canon.objectives).toBeUndefined();
