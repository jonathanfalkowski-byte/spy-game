...
it('uses a stored observation, not private investigations, and limits inquiries', () => {
  const old = information();
  let s = choose(old, 'open-invitation');
  expect(s.npcs.sloane).toEqual(old.npcs.sloane);
  expect(s.day.records.find((r) => r.key === 'c3.referral')?.text).toContain(
    invitationBasis(old).text,
  );
  const altered = structuredClone(old);
  altered.mission.source = 'priya';
  altered.mission.method = 'photo';
...
it.each(['embrace', 'negotiate', 'exploit', 'refuse'])(
  'completes reception %s with earned fee intact',
  (route) => {
    const old = reception();
    let s = choose(old, 'reception-' + route);
    if (s.phase === 'photograph') s = choose(s, 'photo-refuse');
    expect(s.choices['c3.paid']).toBe('600');
    expect(s.proof.find((p) => p.key === 'c3.fee')).toEqual(
      old.proof.find((p) => p.key === 'c3.fee'),
    );
    ...
    if (route === 'exploit')
      expect(s.day.records.some((r) => r.key === 'c3.procurement')).toBe(true);
    if (route === 'negotiate')
      expect(s.day.records.find((r) => r.key === 'c3.introduction')?.text).toBe(
        'Evelynn Vale, our guest today.',
      );
  },
);
it('publishes only the explicitly authorized image and caption without global discovery', () => {
  const old = reception();
  const s = choose(choose(old, 'reception-embrace'), 'photo-publish');
  expect(s.day.records.find((r) => r.key === 'c3.public-association')?.text).toContain(
    publicityCaption,
  );
  expect(s.day.records.find((r) => r.key === 'c3.photo-scope')?.text).toContain('No licensing');
  for (const id of ['sloane', 'maya', 'voss', 'marcus'] as const)
    expect(s.npcs[id]).toEqual(old.npcs[id]);
  expect(decodeSave(encodeSave(s))).toEqual(s);
});
