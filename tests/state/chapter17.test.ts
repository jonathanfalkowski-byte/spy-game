...
const prefer = ['open-room', 'press-fraud', 'sloane-stand', 'offer-refuse', 'named-wait', 'last-no'];
...
it('argues the defect, with the original produced if she only has a reconstruction and an ally has it', () => {
  ...
  expect([cost.phase, cost.choices['act4.press']]).toEqual(['sloane', 'cost']);
...
it('resolves Sloane in front of the board, and lets Evelynn choose what she is there', () => {
  const inRoom = walk(start({ 'act4.inside': 'sloane' }), ['begin', 'open-room', 'press-fraud']);
  expect(ch17(inRoom)).toContain('I objected in writing, on the fourth of March.');
  const annexe = walk(start({ 'act4.inside': 'none' }), ['begin', 'open-room', 'press-fraud']);
...
it('makes the offer, and lands the held card on it', () => {
  ...
  expect([refused.phase, refused.choices['act4.offer'], refused.choices['act4.held-landed']]).toEqual(['nell', 'refuse', 'nell']);
...
it('tells what happened to Nell, and whether Celeste says her name', () => {
  const nell = walk(start({ 'act4.inside': 'nora' }), ['begin', 'open-room', 'press-fraud', 'sloane-stand', 'offer-refuse']);
  ...
  expect(ids(walk(start({ 'act4.inside': 'none' }), ['begin', 'open-room', 'press-fraud', 'sloane-stand', 'offer-refuse']))).toEqual(['named-ask', 'named-wait']);
