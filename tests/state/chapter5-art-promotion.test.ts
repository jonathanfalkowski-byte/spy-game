...
it('packages exact Harbour shots without expanding identity authority or branch coverage', () => {
  expect(harbourApprovals.map((a) => a.shotId)).toEqual([
  ...
  for (const approved of harbourApprovals) {
    ...
    expect(approved.requiredBeatState).toMatchObject({
      event: 'attend',
      wardrobe: 'c05.professional',
      contact: false,
    });
    if (['c05.s06.shot12-entrance', 'c05.s06.shot15-departed'].includes(approved.shotId))
      expect(approved.requiredBeatState).toMatchObject({
        julianAccess: true,
        attention: 'coffee',
        coffeeCount: 1,
      });
    else if (approved.shotId === 'c05.s06.shot01-preview')
      expect(approved.requiredBeatState).toMatchObject({
        arrivalCompleted: true,
        programmeHandoverCompleted: true,
        coffeeCount: 0,
        julianPresent: false,
      });
    else
