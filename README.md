...
New runs use schema version **5**, content version **13**, and `contentRevision: 13`. Content 12 is frozen byte-for-byte from reviewed commit `e40ae0d`, including its full dependency graph; content 11 and older paths retain their previous frozen behavior. Loading authenticates the exact snapshot against its matching ledger and does not write or advance it. At the completed content-12 Chapter 3 Scene 1 checkpoint, an explicit **Continue Chapter 3** choice authenticates the frozen snapshot, verifies the content-13 replay preserves every prefix field except the version marker, and appends the Scene 2 continuation. Older content-11 saves do not silently upgrade.

Chapter 3 Scene 2 now covers Maya contact or deliberate noncontact, sourced Sloane pressure or the restricted-access alternative, explicit channel closure, optional monitored follow-up and overnight rest. Voss’s follow-up and later Chapter 3 scenes remain treatment material. See [the implementation and review report](docs/story/CHAPTER_3_SCENE_2_IMPLEMENTATION.md).
