...
The Glass House uses schema version **5** and content version **9**. Earlier content versions are authenticated against their frozen engines in `src/persistence/legacy-v1` through `legacy-v8`, then the unchanged decision ledger is replayed with current content. Older saves receive explicit empty mission and Celeste defaults. Existing choices, physical progress, evidence custody, and outcomes remain intact. The prose update also corrects the late-capture journal wording to distinguish seeing Benton with the wafer from capturing the handover. Loading alone does not overwrite stored data or advance the story; the next committed action saves the current version.
