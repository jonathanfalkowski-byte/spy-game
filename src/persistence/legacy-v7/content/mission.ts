...
export const missionScenes = [
  scene('car', 'The city goes on', '18:20 · In the car', [
    p(
      'The door closes on Axiom’s service drive. For a moment the car is quiet enough to hear the seat leather settle beneath you. Then the driver indicates, a delivery van lets him in, and the tower begins to slide behind the window.',
    ),
    p(
  ...
  scene('reception', 'A name that opens doors', '18:51 · Reception', [
    p(
      'A woman behind a low stone desk asks for your invitation. There is no turnstile, but a man in a dark suit stands where he can see both your hands. You hold the phone over the reader. The invitation opens to Evelyn’s name and photograph.',
    ),
  ...
  scene('celesteReply', 'What she leaves unsaid', '19:06 · The gathering', [
    ...
    p(
      'He sets his glass on a passing tray and walks away. Celeste follows him with her eyes, then draws her hand back from your arm. She remains by the window. You could find her again without asking anyone where she went.',
    ),
  ]),
  ...
  scene('method', 'What can you bring back?', '19:22 · Before the transfer', [
    q('Sloane · earpiece', 'Decide how you will capture the exchange before you move.'),
    p(
  ...
  scene('debrief', 'The other objective', '19:25 · Descending elevator', [
    p(
      'The doors meet. For the first time since Marcus greeted you, nobody is standing close enough to require an expression. Your hand stays curled around the thing you are holding until you make yourself loosen it.',
    ),
    q(
      'Sloane · earpiece',
  ...
  scene('debriefReply', 'What she calls recoverable', '19:26 · Descending elevator', [
    p(
      'The floor display drops below the reception levels. A little vibration travels through the soles of your shoes. Sloane is a voice in your ear again, unable to see where you put your eyes.',
    ),
  ]),
  ...
  scene('warning3', 'A question to carry', '19:27 · Descending elevator', [
    ...
    t(
      'A stranger can make a question sound like an answer. That does not mean they have given me one.',
    ),
  ]),
  scene('garage', 'Below the glass', '19:28 · Service garage', [
    ...
    p(
      'A car waits beyond the marked pedestrian strip. Its driver lowers the window and gives the return code from your instructions; you compare it before approaching. He opens the rear door. For a moment you stand beside it, breathing air that smells faintly of wet tyres.',
    ),
  ]),
...
> = {
  guest: {
    ...
    limits:
      'The record links Benton to the concealed credential. It does not show a transaction or authenticate the wafer’s contents.',
    blocks: [
      p(
...
for (const [id, label, hint] of [
  ...
  [
    'token',
    'Try to take Benton’s access token',
  ...
  if (phase === 'method')
    blocks.unshift(
      p(
        'You told Sloane: ' +
          sourceNames[m.source!] +
          '. She acknowledges the assessment and asks you to commit a capture method before the meeting begins.',
      ),
    );
  if (phase === 'exchange')
    blocks.push(
      ...
      p(
        m.method === 'audio'
          ...
          : m.method === 'photo'
            ...
            : 'You free your hand and consider the gallery threshold. The object you intend to take is Benton’s access token, not the wafer Marcus will bring.',
      ),
    );
  if (phase === 'confrontation') blocks.push(...captureScene(s));
  if (phase === 'escape') blocks.push(...escapeScene(s));
  if (phase === 'debrief' && m.capture)
    blocks.push(q('Sloane · earpiece', debriefEvidence(s)), p(reasoningText(s)));
  return blocks;
}
...
export function debriefEvidence(s: GameState): string {
  ...
  return (
    ...
    ' ' +
    (c.owner === 'Sloane'
      ? 'My recorder holds the copy.'
      : c.owner === 'Evelyn'
        ? 'You retain it for now.'
        : 'You obtained no item to bring back.')
  );
}
