...
export function applyDayChoice(state: GameState, s: GameState, id: string): boolean {
  ...
  if (['file.report', 'file.delete', 'file.trace'].includes(id)) {
    ...
    s.feedback = text;
    s.history.push({ node: 'file.arrival', blocks: [{ kind: 'notice', text }] });
  }
  if (id === 'file.open') {
  ...
  if (id.startsWith('closure.')) {
    ...
    if (value === 'checkin')
      maya(
        'Check Adrian at 06:30; look for Sublevel 17 if he does not answer',
  ...
  s.history.push({ node: c.next, blocks: dayBlocks(s) });
  return true;
}
