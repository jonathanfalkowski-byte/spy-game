import { applyChapter4Choice as applyFrozenChapter4 } from '../../legacy-v15/content/chapter4';
import { applyNextChoice as applyFrozenNext } from '../../legacy-v14/content/chapter3-next';
import { canContinueAudit } from './audit-continuation';
import { applyChapter5Choice as applyFrozenChapter5 } from '../../legacy-v16/content/chapter5';
import { chapter5Choices, applyChapter5Choice } from '../content/chapter5';
import { replay as replayV15, reducer as reducerV15, availableIntents as intentsV15 } from '../../legacy-v15/state/reducer';
import { chapter4Choices, applyChapter4Choice } from '../content/chapter4';
import { replay as replayV14, reducer as reducerV14, availableIntents as intentsV14 } from '../../legacy-v14/state/reducer';
import { replay as replayV13 } from '../../legacy-v13/state/reducer';
import { nextChoices, applyNextChoice } from '../content/chapter3-next';
import { initialState as legacyInitialState, reducer as legacyReducer, replay as legacyReplay, availableIntents as legacyIntents } from '../../legacy-v11/state/reducer';
import { initialState as initialV12, reducer as reducerV12, replay as replayV12, availableIntents as intentsV12 } from '../../legacy-v12/state/reducer';
...
export function reducer(state: GameState, input: unknown): GameState {
  ...
  if (state.contentRevision === 16 || state.contentRevision === 15) {
    ...
    if(parsed.success && parsed.data.type==='CHAPTER5_CHOOSE' && parsed.data.expectedRevision===state.revision){
      ...
      return applyFrozenChapter5(state as Parameters<typeof applyFrozenChapter5>[0],parsed.data.id) as GameState;
    }
