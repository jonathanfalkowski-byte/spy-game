...
export function dayBlocks(state: GameState): Block[] {
  ...
  const prepend: Block[] = [];
  if (node === 'file.arrival' && d.directorySeen) {
    base.splice(0, base.length,
      p('The directory is closed. Its entry remains in your workspace, beside the Helix report. You are back at the anomaly, with the identifiers you saw still in mind.'),
      p('You have not authorized the candidate record. You can make any remaining checks, reopen the directory, or leave it untouched.'),
    );
  }
