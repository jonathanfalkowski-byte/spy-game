...
import { chapter13Choices } from '../content/chapter13';
import { displayName, renderChoiceText } from './reading-presentation';
export function Chapter13work({ state, send }: { state: GameState; send: (a: Intent) => void }) {
  const node = `${state.scene}.${state.phase}`;
  const choices = chapter13Choices(state);
  return choices.length ? (
    <section className="decision" aria-label="Chapter 13 action">
      <span className="eyebrow">Your next action</span>
      <div className="choice-list">
        {choices.map((c) => (
          <button key={c.id} data-chapter13-choice={c.id} onClick={() => send({ type: 'CHAPTER13_CHOOSE', id: c.id })}>
