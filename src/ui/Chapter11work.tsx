...
import { chapter11Choices } from '../content/chapter11';
import { displayName, renderChoiceText } from './reading-presentation';
export function Chapter11work({ state, send }: { state: GameState; send: (a: Intent) => void }) {
  const node = `${state.scene}.${state.phase}`;
  const choices = chapter11Choices(state);
  return choices.length ? (
    <section className="decision" aria-label="Chapter 11 action">
      <span className="eyebrow">Your next action</span>
      <div className="choice-list">
        {choices.map((c) => (
          <button key={c.id} data-chapter11-choice={c.id} onClick={() => send({ type: 'CHAPTER11_CHOOSE', id: c.id })}>
