...
import { chapter8Choices } from '../content/chapter8';
import { displayName, renderChoiceText } from './reading-presentation';
export function Chapter8work({ state, send }: { state: GameState; send: (a: Intent) => void }) {
  const node = `${state.scene}.${state.phase}`;
  const choices = chapter8Choices(state);
  return choices.length ? (
    <section className="decision" aria-label="Chapter 8 action">
      <span className="eyebrow">Your next action</span>
      <div className="choice-list">
        {choices.map((c) => (
          <button key={c.id} data-chapter8-choice={c.id} onClick={() => send({ type: 'CHAPTER8_CHOOSE', id: c.id })}>
