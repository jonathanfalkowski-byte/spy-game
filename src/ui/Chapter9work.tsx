...
import { chapter9Choices } from '../content/chapter9';
import { displayName, renderChoiceText } from './reading-presentation';
export function Chapter9work({ state, send }: { state: GameState; send: (a: Intent) => void }) {
  const node = `${state.scene}.${state.phase}`;
  const choices = chapter9Choices(state);
  return choices.length ? (
    <section className="decision" aria-label="Chapter 9 action">
      <span className="eyebrow">Your next action</span>
      <div className="choice-list">
        {choices.map((c) => (
          <button key={c.id} data-chapter9-choice={c.id} onClick={() => send({ type: 'CHAPTER9_CHOOSE', id: c.id })}>
