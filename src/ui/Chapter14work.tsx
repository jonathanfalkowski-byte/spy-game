...
import { chapter14Choices } from '../content/chapter14';
import { displayName, renderChoiceText } from './reading-presentation';
export function Chapter14work({ state, send }: { state: GameState; send: (a: Intent) => void }) {
  const node = `${state.scene}.${state.phase}`;
  const choices = chapter14Choices(state);
  return choices.length ? (
    <section className="decision" aria-label="Chapter 14 action">
      <span className="eyebrow">Your next action</span>
      <div className="choice-list">
        {choices.map((c) => (
          <button key={c.id} data-chapter14-choice={c.id} onClick={() => send({ type: 'CHAPTER14_CHOOSE', id: c.id })}>
