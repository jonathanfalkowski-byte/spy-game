...
import { chapter17Choices } from '../content/chapter17';
import { displayName, renderChoiceText } from './reading-presentation';
export function Chapter17work({ state, send }: { state: GameState; send: (a: Intent) => void }) {
  const node = `${state.scene}.${state.phase}`;
  const choices = chapter17Choices(state);
  return choices.length ? (
    <section className="decision" aria-label="Chapter 17 action">
      <span className="eyebrow">Your next action</span>
      <div className="choice-list">
        {choices.map((c) => (
          <button key={c.id} data-chapter17-choice={c.id} onClick={() => send({ type: 'CHAPTER17_CHOOSE', id: c.id })}>
