...
import { chapter6Choices } from '../content/chapter6';
import { displayName, renderChoiceText } from './reading-presentation';
export function Chapter6work({ state, send }: { state: GameState; send: (a: Intent) => void }) {
  const node = `${state.scene}.${state.phase}`;
  const choices = chapter6Choices(state);
  return choices.length ? (
    <section className="decision" aria-label="Chapter 6 action">
      <span className="eyebrow">Your next action</span>
      <div className="choice-list">
        {choices.map((c) => (
          <button key={c.id} data-chapter6-choice={c.id} onClick={() => send({ type: 'CHAPTER6_CHOOSE', id: c.id })}>
