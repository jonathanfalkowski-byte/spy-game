...
import { chapter16Choices } from '../content/chapter16';
import { displayName, renderChoiceText } from './reading-presentation';
export function Chapter16work({ state, send }: { state: GameState; send: (a: Intent) => void }) {
  const node = `${state.scene}.${state.phase}`;
  const choices = chapter16Choices(state);
  return choices.length ? (
    <section className="decision" aria-label="Chapter 16 action">
      <span className="eyebrow">Your next action</span>
      <div className="choice-list">
        {choices.map((c) => (
          <button key={c.id} data-chapter16-choice={c.id} onClick={() => send({ type: 'CHAPTER16_CHOOSE', id: c.id })}>
