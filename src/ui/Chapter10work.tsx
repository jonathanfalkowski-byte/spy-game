...
import { chapter10Choices } from '../content/chapter10';
import { displayName, renderChoiceText } from './reading-presentation';
export function Chapter10work({ state, send }: { state: GameState; send: (a: Intent) => void }) {
  const node = `${state.scene}.${state.phase}`;
  const choices = chapter10Choices(state);
  return choices.length ? (
    <section className="decision" aria-label="Chapter 10 action">
      <span className="eyebrow">Your next action</span>
      <div className="choice-list">
        {choices.map((c) => (
          <button key={c.id} data-chapter10-choice={c.id} onClick={() => send({ type: 'CHAPTER10_CHOOSE', id: c.id })}>
