...
export function Chapter3work({ state, send }: { state: GameState; send: (a: Intent) => void }) {
  if (availableIntents(state).some(a => a.type === 'CONTINUE_CHAPTER3_SCENE2')) return <section className="decision" aria-label="Chapter 3 continuation"><button onClick={() => send({type:'CONTINUE_CHAPTER3_SCENE2'})}>Continue Chapter 3 · What you can tell her</button><p>Continue into Scene 2. Your earlier decisions and history remain intact.</p></section>;
