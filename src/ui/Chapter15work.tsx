...
import { chapter15Choices } from '../content/chapter15';
import { displayName, renderChoiceText } from './reading-presentation';
export function Chapter15work({ state, send }: { state: GameState; send: (a: Intent) => void }) {
  const node = `${state.scene}.${state.phase}`;
  const choices = chapter15Choices(state);
  return choices.length ? (
    <section className="decision" aria-label="Chapter 15 action">
      <span className="eyebrow">Your next action</span>
      <div className="choice-list">
        {choices.map((c) => (
          <button key={c.id} data-chapter15-choice={c.id} onClick={() => send({ type: 'CHAPTER15_CHOOSE', id: c.id })}>
