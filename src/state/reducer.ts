...
import { chapter6Choices, applyChapter6Choice } from '../content/chapter6';
import { chapter7Choices, applyChapter7Choice } from '../content/chapter7';
import { chapter8Choices, applyChapter8Choice } from '../content/chapter8';
import { chapter9Choices, applyChapter9Choice } from '../content/chapter9';
...
import { chapter14Choices, applyChapter14Choice } from '../content/chapter14';
import { chapter15Choices, applyChapter15Choice } from '../content/chapter15';
import { chapter16Choices, applyChapter16Choice } from '../content/chapter16';
import { chapter17Choices, applyChapter17Choice } from '../content/chapter17';
import { chapter18Choices, applyChapter18Choice } from '../content/chapter18';
...
export function reducer(state: GameState, input: unknown): GameState {
  ...
  switch (action.type) {
    ...
    case 'CHAPTER6_CHOOSE':
      return applyChapter6Choice(state, action.id);
    case 'CHAPTER7_CHOOSE':
      return applyChapter7Choice(state, action.id);
    case 'CHAPTER8_CHOOSE':
      return applyChapter8Choice(state, action.id);
    case 'CHAPTER9_CHOOSE':
      return applyChapter9Choice(state, action.id);
    case 'CHAPTER10_CHOOSE':
      return applyChapter10Choice(state, action.id);
    case 'CHAPTER11_CHOOSE':
    ...
    case 'CHAPTER14_CHOOSE':
      return applyChapter14Choice(state, action.id);
    case 'CHAPTER15_CHOOSE':
      return applyChapter15Choice(state, action.id);
    case 'CHAPTER16_CHOOSE':
      return applyChapter16Choice(state, action.id);
    case 'CHAPTER17_CHOOSE':
      return applyChapter17Choice(state, action.id);
    case 'CHAPTER18_CHOOSE':
      return applyChapter18Choice(state, action.id);
    case 'MISSION_CHOOSE':
...
function storyIntents(s: GameState): Intent[] {
  ...
  intents.push(...chapter6Choices(s).map((c) => ({ type: 'CHAPTER6_CHOOSE' as const, id: c.id })));
  intents.push(...chapter7Choices(s).map((c) => ({ type: 'CHAPTER7_CHOOSE' as const, id: c.id })));
  intents.push(...chapter8Choices(s).map((c) => ({ type: 'CHAPTER8_CHOOSE' as const, id: c.id })));
  intents.push(...chapter9Choices(s).map((c) => ({ type: 'CHAPTER9_CHOOSE' as const, id: c.id })));
  ...
  intents.push(...chapter14Choices(s).map((c) => ({ type: 'CHAPTER14_CHOOSE' as const, id: c.id })));
  intents.push(...chapter15Choices(s).map((c) => ({ type: 'CHAPTER15_CHOOSE' as const, id: c.id })));
  intents.push(...chapter16Choices(s).map((c) => ({ type: 'CHAPTER16_CHOOSE' as const, id: c.id })));
  intents.push(...chapter17Choices(s).map((c) => ({ type: 'CHAPTER17_CHOOSE' as const, id: c.id })));
  intents.push(...chapter18Choices(s).map((c) => ({ type: 'CHAPTER18_CHOOSE' as const, id: c.id })));
